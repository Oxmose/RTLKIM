...
/** @brief Current page dierctory */
extern uint64_t* kernel_pgdir;
...
extern void __cpu_smp_loader_init();
 ...
 * @param stack_state The stack state before the exception that contain cs, rip,
 * error code and the rflags register value.
 */
...
{
    /* Set ERP, RSP and EBP */
    thread->cpu_context.rip = (uintptr_t)entry_point;
    thread->cpu_context.rsp = (uintptr_t)&thread->stack[stack_index - 27];
    thread->cpu_context.rbp = (uintptr_t)&thread->stack + stack_index - 1;
    ...
    /* Init thread stack */
    thread->stack[stack_index - 1]  = THREAD_INIT_SS;
    thread->stack[stack_index - 2]  = thread->cpu_context.rbp;
    thread->stack[stack_index - 3]  = THREAD_INIT_RFLAGS;
    thread->stack[stack_index - 4]  = THREAD_INIT_CS;
    thread->stack[stack_index - 5]  = thread->cpu_context.rip;
    thread->stack[stack_index - 6]  = 0; /* UNUSED (error) */
    thread->stack[stack_index - 7]  = 0; /* UNUSED (int id) */
    thread->stack[stack_index - 8]  = THREAD_INIT_DS;
    thread->stack[stack_index - 9]  = THREAD_INIT_ES;
    thread->stack[stack_index - 10] = THREAD_INIT_FS;
    thread->stack[stack_index - 11] = THREAD_INIT_GS;
    thread->stack[stack_index - 12] = THREAD_INIT_SS;
    thread->stack[stack_index - 13] = THREAD_INIT_R8;
    thread->stack[stack_index - 14] = THREAD_INIT_R9;
    thread->stack[stack_index - 15] = THREAD_INIT_R10;
    thread->stack[stack_index - 16] = THREAD_INIT_R11;
    thread->stack[stack_index - 17] = THREAD_INIT_R12;
    thread->stack[stack_index - 18] = THREAD_INIT_R13;
    thread->stack[stack_index - 19] = THREAD_INIT_R14;
    thread->stack[stack_index - 20] = THREAD_INIT_R15;
    thread->stack[stack_index - 21] = THREAD_INIT_RAX;
    thread->stack[stack_index - 22] = THREAD_INIT_RBX;
    thread->stack[stack_index - 23] = THREAD_INIT_RCX;
    thread->stack[stack_index - 24] = THREAD_INIT_RDX;
    thread->stack[stack_index - 25] = THREAD_INIT_RSI;
    thread->stack[stack_index - 26] = THREAD_INIT_RDI;
    thread->stack[stack_index - 27] = thread->cpu_context.rbp;
    thread->stack[stack_index - 28] = thread->cpu_context.rsp;
}
...
{
    (void)stack_state;
    /* Save the actual RSP (not the fist time since the first schedule should
     * dissociate the boot sequence (pointed by the current RSP) and the IDLE
     * thread.*/
    if(first_sched == 1)
    {
        thread->cpu_context.rsp = cpu_state->rsp;
    }
}
                         ...
                         const kernel_thread_t* thread)
{
    ...
    /* Update rsp */
    cpu_state->rsp = thread->cpu_context.rsp;
...
{
    /* Update CR3 */
    __asm__ __volatile__("mov %%rax, %%cr3": :"a"(new_pgdir));
}
...
{
    return ((cpu_save_flags() & CPU_RFLAGS_IF) != 0);
}
...
{
    (void) cpu_state;
    return stack_state->rflags & CPU_RFLAGS_IF;
}
