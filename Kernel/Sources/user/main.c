...
void* sem_thread_routine(void* args)
{
    ...
    {
        sem_pend(&sem);
        printf("%u ", (uint32_t)(uintptr_t)args);
    }
...
int sem_ex(void)
{
    ...
    {
        err = sched_create_kernel_thread(&threads[i], i % 10, "sem_ex", 0x2000, 0,
                                  sem_thread_routine, (void*)(uintptr_t)i);
        if(err != OS_NO_ERR)
        {
...
int main(void)
{
    ...
    {
        sched_create_kernel_thread(&threads[i], 0, "thread T", 0x2000,
                                   i, thread_routine, (void*)(uintptr_t)i);
    }
    for(uint32_t i = 0; i < MAX_CPU_COUNT; ++i)
    {
