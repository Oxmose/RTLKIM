...
static uint32_t             rsdp_parse_success;
...
static OS_RETURN_E acpi_parse_apic(acpi_madt_t* madt_ptr)
{
    ...
    kernel_serial_debug("Adding MADT fault handler 0x%p -> 0x%p\n", madt_ptr, madt_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)madt_ptr,
        (address_t)(madt_ptr + 1));
...
static OS_RETURN_E acpi_parse_facs(acpi_facs_t* facs_ptr)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding FACS fault handler 0x%p -> \n", facs_ptr, facs_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)facs_ptr,
        (address_t)(facs_ptr + 1));
...
static OS_RETURN_E acpi_parse_dsdt(acpi_dsdt_t* dsdt_ptr)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding DSDT fault handler 0x%p -> 0x%p\n", dsdt_ptr, dsdt_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)dsdt_ptr,
        (address_t)(dsdt_ptr + 1));
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding DSDT 2 fault handler 0x%p -> 0x%p\n", (dsdt_ptr + 1), (address_t)(dsdt_ptr) + dsdt_ptr->header.length);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)(dsdt_ptr + 1),
        (address_t)(dsdt_ptr) + dsdt_ptr->header.length);
...
static OS_RETURN_E acpi_parse_fadt(acpi_fadt_t* fadt_ptr)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding FADT fault handler 0x%p -> 0x%p\n", (address_t)fadt_ptr + (sizeof(acpi_header_t)), fadt_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)fadt_ptr + (sizeof(acpi_header_t)),
        (address_t)(fadt_ptr + 1));
    if(err != OS_NO_ERR)
    {
        return err;
    }
...
static OS_RETURN_E acpi_parse_dt(acpi_header_t* header)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding DT fault handler 0x%p -> 0x%p\n", header, header + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)header,
...
static OS_RETURN_E acpi_parse_rsdt(rsdt_descriptor_t* rsdt_ptr)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding RSDT fault handler 0x%p -> 0x%p\n", rsdt_ptr, rsdt_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)rsdt_ptr,
        (address_t)(rsdt_ptr + 1));
    if(err != OS_NO_ERR)
    {
        return err;
    }
...
static OS_RETURN_E acpi_parse_xsdt(xsdt_descriptor_t* xsdt_ptr)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding XSDT fault handler 0x%p -> 0x%p\n", xsdt_ptr, xsdt_ptr + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)xsdt_ptr,
        (address_t)(xsdt_ptr + 1));
...
static OS_RETURN_E acpi_parse_rsdp(rsdp_descriptor_t* rsdp_desc)
{
    ...
    #if ACPI_KERNEL_DEBUG == 1
    kernel_serial_debug("Adding RSDP fault handler 0x%p -> 0x%p\n", rsdp_desc, rsdp_desc + 1);
    #endif
    err = paging_register_fault_handler(
        acpi_page_fault_handler, 
        (address_t)rsdp_desc,
        (address_t)(rsdp_desc + 1));
    if(err != OS_NO_ERR)
    {
        return err;
    }
    ...
    else if(rsdp_desc->revision == 2)
    {
        ...
        if(xsdt_addr)
        {
            err = acpi_parse_xsdt((xsdt_descriptor_t*)(address_t)xsdt_addr);
            if(err != OS_NO_ERR)
            {
                return err;
            }
