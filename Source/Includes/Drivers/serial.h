...
/* Init serial driver intrrupts on input.
 *
 * @returns OS_NO_ERR if successfull. Otherwise an error is returned.
 */
OS_RETURN_E init_serial_interrupt(void);
...
/* Tells if the data on the serial port are ready to be read.
 *
 * @param port The serial port on which the test should be executed.
 * @returns 1 is returned if data can be read from the port. 0 is returned
 * otherwise.
 */
uint8_t serial_received(const uint32_t port);

/* Read a byte from the serial port given as parameter.
 *
